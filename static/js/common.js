...
const listInfo_toc = (item) => {
  ...
  l.append(s);
  l.append(
    '<div class="toc-edit"><span class="oi oi-pencil" data-edit="false"></span><span class="oi oi-trash"></span></div>'
  );
  return l;
};
...
$("#microcontentModal").on("click", "#microcontent_edit_save", function () {
  ...
  book.skill = $(".skill-list .skill:checked")
    .map(function () {
      return $(this).val();
    })
    .get();

  book.task = $(".task-list .task:checked")
    .map(function () {
      return $(this).val();
    })
    .get();

  book.level = $(".level-list .level:checked")
    .map(function () {
      return $(this).val();
    })
    .get();
